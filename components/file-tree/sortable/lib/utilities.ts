...
function flatten(
  ...
  //@ts-ignore Fix parent id typing
...
export function buildTree(flattenedItems: FlattenedItem[]): TreeItems {
  ...
  for (const item of items) {
    ...
    nodes[id] = { id, label, type, children };
...
export function setCollapsed(
  ...
  const updateNode = (nodes: TreeItems): TreeItems =>
    nodes.map((node) => ({
      ...node,
      ...(node.id === id && { collapsed: !node.collapsed }),
      ...(node.children && { children: updateNode(node.children) }),
    }));

  return updateNode(items);
}
